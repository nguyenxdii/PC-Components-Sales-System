...
const CheckoutPage = () => {
  ...
  const handleApplyVoucher = async () => {
    ...
    try {
      ...
      if (isFreeShip) {
        message.success(`Áp dụng mã thành công! Bạn được Miễn phí vận chuyển cho đơn hàng này.`);
      } else {
        message.success(`Áp dụng mã thành công! Bạn được giảm ${formatPrice(discountAmount)}`);
      }
    } catch (error) {
      setDiscountAmount(0);
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="container mx-auto px-4">
        ...
        <Title level={2} className="!mb-8 font-black uppercase tracking-tight">Thanh toán đơn hàng</Title>
          ...
          <div className="grid grid-cols-12 gap-8 items-start">
            ...
            <div className="col-span-12 lg:col-span-5">
              <Card className="rounded-3xl shadow-lg border-none">
                <Title level={4} className="!mb-6 uppercase text-sm font-black text-gray-400 tracking-widest">Đơn hàng của bạn</Title>
                ...
                <div className="bg-gray-50 p-4 rounded-2xl mb-6">
                  <Form.Item name="voucherCode" label={<span className="text-xs font-bold uppercase text-gray-400">Mã giảm giá (Voucher)</span>} className="mb-0">
                    <Input 
                      ...
                      className="h-12 rounded-xl border-dashed border-2 hover:border-primary transition-colors"
                      suffix={
                        <Button 
                          type="link" 
                          onClick={handleApplyVoucher} 
                          loading={applyingVoucher}
                          className="font-bold uppercase text-[10px] p-0 h-auto"
                        >
                          ÁP DỤNG
                        </Button>
                      }
                ...
                <div className="space-y-3">
                   ...
                   <Button 
                    ...
                    loading={loading}
                    htmlType="submit"
                    className="h-16 mt-8 rounded-2xl font-black text-lg shadow-xl shadow-red-100 border-none bg-gradient-to-r from-red-600 to-orange-500 hover:scale-[1.02] transition-transform"
                  >
                    XÁC NHẬN ĐẶT HÀNG
                  </Button>
