...
const ProductDetail = () => {
    ...
    const fetchProductAndComments = async () => {
      ...
      try {
        ...
        if (userData) {
          ...
          setUser(parsedUser);
          const eligible = await commentService.checkEligibility(parsedUser.id, prodData.id);
        ...
        if (prodData) {
          try {
            const commentData = await commentService.getCommentsByProductId(prodData.id);
            setComments(commentData);
          } catch (commentError) {
            console.error("Lỗi khi tải bình luận:", commentError);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (replyId && comments.length > 0) {
        const targetId = parseInt(replyId);
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      setReplyingTo(null);
      // Refresh comments
      const commentData = await commentService.getCommentsByProductId(product.id);
      setComments(commentData);
    } catch (error) {
  ...
  const CommentItem = ({ 
    ...
    const isTargeted = location.hash === `#comment-${comment.id}` || 
                       new URLSearchParams(location.search).get("replyTo") === String(comment.id);
          ...
          <div className="flex gap-4 mb-2">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between gap-2 mb-1">
                <Text strong className="text-gray-900">{comment.userName}</Text>
              ...
              <div className="bg-white/50 p-4 rounded-2xl relative border border-gray-100/50 shadow-sm">
                ...
                {/* Reply Button for Everyone (Logged in) */}
                {user && (
                    <Button 
                        type="link" 
                        size="small" 
              ...
              {user && replyingTo?.id === comment.id && (
                  <div className="mt-4 p-4 bg-gray-50 rounded-2xl border border-gray-100 shadow-inner">
                      <TextArea
                          ...
                          rows={3}
                          placeholder={`Phản hồi cho ${comment.userName}...`}
                          value={newComment}
          ...
          <div className="mb-12 bg-gray-50 p-6 rounded-2xl border border-gray-100">
            ...
            {replyingTo ? (
                <div className="mb-4 flex items-center justify-between bg-blue-50 p-3 rounded-lg border border-blue-100">
                    <Text className="text-blue-600 font-medium whitespace-nowrap overflow-hidden text-ellipsis">
                        Đang phản hồi cho <b>{replyingTo.userName}</b>: <span className="italic text-xs">"{replyingTo.content.substring(0, 50)}..."</span>
                    </Text>
                    <Button 
                        type="link" 
                        danger 
                ...
                <div className="mb-4">
                    <Text className="mr-4 font-bold text-gray-600 uppercase text-xs tracking-widest">Đánh giá sao:</Text>
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="container mx-auto px-4 py-8">
        <Breadcrumb className="mb-6 breadcrumb-premium" items={[
          ...
          { title: <span className="opacity-60">{product.category?.name}</span> },
          { title: <span className="text-gray-900">{product.name}</span> }
        ]} />
        ...
        <div className="grid grid-cols-12 gap-10">
          ...
          <div className="col-span-12 lg:col-span-7">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 h-full">
              ...
              <Title level={2} className="!mb-2 !text-3xl font-extrabold text-gray-900">
                {product.name}
              </Title>
