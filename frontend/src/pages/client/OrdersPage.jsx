        ...
        return (
          <Space direction="vertical" size={4}>
            ...
            <Button 
              ...
              type="primary" 
              ghost 
              icon={<SyncOutlined spin={checkingOrderId === record.id} />} 
              className="text-[10px] h-6 px-2 rounded-md"
              onClick={() => handleSingleCheck(record)}
              loading={checkingOrderId === record.id}
            >
              Kiểm tra ngay
            </Button>
          </Space>
            ...
            <div className="space-y-6 pt-4">
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                <div>
                  ...
                  <Text strong className="block">{selectedOrder.user?.fullName}</Text>
                  <Text className="block text-xs text-gray-500">{selectedOrder.receiverPhone}</Text>
                  <Text className="block text-xs text-gray-500">{selectedOrder.shippingAddress}</Text>
                </div>
                <div>
                  <Text type="secondary" className="block mb-1 uppercase text-xs font-bold tracking-wider">Trạng thái & Thanh toán</Text>
                  <div className="mb-2">{getStatusTag(selectedOrder.status)}</div>
                  <Text className="block text-xs text-gray-500">Phương thức: <Tag color="orange" size="small">{selectedOrder.payment?.paymentMethod}</Tag></Text>
                  <Text className="block text-xs text-gray-500">Thanh toán: <Tag color={selectedOrder.payment?.status === 'PAID' ? 'green' : 'red'}>{selectedOrder.payment?.status}</Tag></Text>
                </div>
              </div>
                ...
                columns={[
                  {
                    ...
                    render: (product) => (
                      <Link to={`/product/${product?.slug}`} className="hover:opacity-80 transition-opacity">
                        <Space>
                          {product?.imageUrl && <img src={product.imageUrl} alt={product.name} className="w-10 h-10 object-contain rounded border" />}
                          <Text strong className="text-xs text-blue-600 hover:text-blue-800">{product?.name || 'Sản phẩm không xác định'}</Text>
                        </Space>
                      </Link>
                    )
                  },
                  ...
                  },
                  {
                    ...
                    render: (_, record) => (
                      <Text strong className="text-xs">
                        {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(record.priceAtPurchase * record.quantity)}
                      </Text>
                    )
                  },
                  {
                    ...
                    render: (_, record) => (
                      (selectedOrder.status === 'DELIVERED' || selectedOrder.status === 'PROCESSING') && (
                        <Link to={`/product/${record.product?.slug}`}>
                          <Button size="small" type="primary" ghost className="text-[10px] h-6 px-2 rounded-md">
                            Đánh giá
                          </Button>
                        </Link>
                      )
              ...
              <div className="text-right space-y-1 pr-4 pt-4 border-t">
                <div>
                  ...
                  <Text strong className="text-xs">{new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(selectedOrder.totalPrice)}</Text>
                </div>
                <div>
                  <Text type="secondary" className="text-xs">Phí vận chuyển: </Text>
                  <Text strong className="text-xs">
                    {selectedOrder.shippingFee === 0 ? "Miễn phí" : new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(selectedOrder.shippingFee)}
                  </Text>
                </div>
                <div className="pt-2">
                  <Text className="text-md">Tổng thanh toán: </Text>
                  <Text className="text-lg font-black text-red-600">
                    {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(selectedOrder.finalPrice)}
                  </Text>
                </div>
              </div>
            </div>
